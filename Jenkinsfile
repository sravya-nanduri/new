    ...
    agent any
        stages{
            stage('deployment'){         
                ...
                sh 'ssh jenkins@13.127.219.105 "cd /var/www/html/ && sudo docker build -t smartims${BUILD_NUMBER} . &&  sudo docker rm smartims -f && sudo docker run -td --name smartims -p 9000:80 smartims"'
                    }
                  }
                  script {
                    if (env.Env == 'qa') {
