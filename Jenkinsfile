    ...
    agent any
        stages{
            stage('deployment'){         
                ...
                sh 'ssh jenkins@13.127.219.105 "cd /var/www/html/ && sudo docker build -t smartims$env.env$BUILD_NUMBER Dockerfile.dev &&  sudo docker rm smartims$env.env -f && sudo docker run -td --name smartims$env.env -p 9001:80 smartimsenv.env$BUILD_NUMBER"'
                    }
                  }
                  script {
                    if (env.Env == 'qa') {
